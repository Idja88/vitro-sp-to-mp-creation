...
class VitroAutomation:
    ...
    def __init__(self, spreadsheet_id: str):
        ...
        self.google_api_delay = (
            1.0  # 1 second between batch updates to stay under 60 req/min
        )
    ...
    def load_constants(self):
        ...
        self.SHEET_CTYPES_TO_ATTRIBUTES_UNIQUE = os.getenv(
            "SHEET_CTYPES_TO_ATTRIBUTES_UNIQUE"
        )
    ...
    def queue_log_message(
        self,
        sheet_name: str,
        row_index: int,
        message: str,
        column_name: str = "SYNC_LOG",
    ):
            ...
            for chunk_idx in range(0, len(all_data), chunk_size):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        print(
                            f"{sheet_name}: chunk {chunk_num}/{total_chunks} ({len(chunk)} cells) updated"
                        )
                        ...
                        if attempt < max_retries - 1:
                            ...
                            print(
                                f"{sheet_name}: chunk {chunk_num} failed, retrying in {wait_time}s... (attempt {attempt + 1}/{max_retries})"
                            )
                            time.sleep(wait_time)
                        else:
                            print(
                                f"{sheet_name}: chunk {chunk_num} failed after {max_retries} attempts: {e}"
                            )
        ...
        for idx, record in enumerate(records, start=2):  # Start at row 2 (after header)
            try:
                ...
                if response and response.get("id"):
                    ...
                    list_of_lists_id = (
                        response.get("fieldValueMap").get("list").get("id")
                    )
        ...
        for idx, record in enumerate(records, start=2):  # Start at row 2 (after header)
            try:
                ...
                data = {
                    ...
                    "hidden": self.convert_value(
                        record.get("MP_LIST_IS_HIDDEN"), "bool"
                    ),
                    "document_archive": self.convert_value(
                        record.get("MP_LIST_IS_STORAGE"), "bool"
                    ),
                    "description": self.DESCRIPTION,
                }
    ...
    def stage_2_create_ctypes(self):
        ...
        for idx, record in enumerate(records, start=2):
            try:
                ...
                is_document = self.convert_value(
                    record.get("MP_CTYPE_IS_DOCUMENT"), "bool"
                )
                parent_id = (
                    self.CTYPES_DEFAULT_DOCUMENT_ID
                    if is_document
                    else self.CTYPES_DEFAULT_ELEMENT_ID
                )
                ...
                data = {
                    ...
                    "folder": self.convert_value(
                        record.get("MP_CTYPE_IS_FOLDER"), "bool"
                    ),
                    "description": self.DESCRIPTION,
                }
                ...
                if response and response.get("id"):
                    ...
                    self.log_to_sheet(
                        ...
                        idx,
                        f"Content type created: {ctype_id}",
                    )
                else:
                    self.log_to_sheet(
                        self.SHEET_CTYPES_UNIQUE, idx, f"ERROR: {response}"
                    )
        ...
        for idx, record in enumerate(records, start=2):
            try:
                ...
                if target_type == "String":
                    default_string = (
                        record.get("default_value_string", "").strip()
                        if isinstance(record.get("default_value_string"), str)
                        else record.get("default_value_string")
                    )
                    if default_string:
                        data["default_value_string"] = default_string

                    letters_max = self.convert_value(
                        record.get("letters_max_count"), "int"
                    )
                    ...
                    default_int = self.convert_value(
                        record.get("default_value_int"), "int"
                    )
                    ...
                    min_decimal = self.convert_value(
                        record.get("min_decimal_value"), "float"
                    )
                    if min_decimal is not None:
                        data["min_decimal_value"] = min_decimal

                    max_decimal = self.convert_value(
                        record.get("max_decimal_value"), "float"
                    )
                    if max_decimal is not None:
                        data["max_decimal_value"] = max_decimal

                    char_after_decimal = self.convert_value(
                        record.get("characters_number_after_decimal_point"), "int"
                    )
                    if char_after_decimal is not None:
                        data["characters_number_after_decimal_point"] = (
                            char_after_decimal
                        )

                    default_decimal = self.convert_value(
                        record.get("default_value_decimal"), "float"
                    )
                    if default_decimal is not None:
                        data["default_value_decimal"] = default_decimal

                elif target_type == "Note":
                    change_lines = self.convert_value(
                        record.get("change_lines_number"), "int"
                    )
                    ...
                    rich_text_toolbar = self.convert_value(
                        record.get("rich_text_toolbar_enabled"), "bool"
                    )
                    if rich_text_toolbar is not None:
                        data["rich_text_toolbar_enabled"] = rich_text_toolbar

                elif target_type == "Datetime":
                    default_date = self.convert_to_iso8601(
                        record.get("default_value_date")
                    )
                    if default_date:
                        data["default_value_date"] = default_date

                elif target_type == "Boolean":
                    default_bool = self.convert_value(
                        record.get("default_value_boolean"), "bool"
                    )
                    ...
                    default_guid = (
                        record.get("default_value_guid", "").strip()
                        if isinstance(record.get("default_value_guid"), str)
                        else record.get("default_value_guid")
                    )
                    if default_guid:
                    ...
                    search_symbol_count = self.convert_value(
                        record.get("search_symbol_count"), "int"
                    )
                    ...
                    list_filter_query = record.get(
                        "lookup_field_list_filter_query", ""
                    ).strip()
                ...
                if response and response.get("id"):
                    ...
                    self.log_to_sheet(
                        ...
                        idx,
                        f"Attribute created: {attr_id}",
                    )
                else:
                    self.log_to_sheet(
                        self.SHEET_ATTRIBUTES_UNIQUE, idx, f"ERROR: {response}"
                    )
    ...
    def stage_4_add_attributes_to_ctypes(self):
        ...
        for idx, record in enumerate(records, start=2):
            try:
                ...
                if (
                    mp_internal_name.lower() == "name"
                    or mp_internal_name.lower() == "file_leaf_ref"
                ):
                    self.log_to_sheet(
                        self.SHEET_CTYPES_TO_ATTRIBUTES_UNIQUE,
                        idx,
                        "Skipped because MP_ATTRIBUTE_INTERNAL_NAME is 'name' or 'file_leaf_ref'",
                    )
                    continue
                ...
                if not ctype_id:
                    self.log_to_sheet(
                        self.SHEET_CTYPES_TO_ATTRIBUTES_UNIQUE,
                        idx,
                        "WARNING: MP_CTYPE_ID is empty in current row",
                    )
                    continue
                ...
                if not attr_id:
                    self.log_to_sheet(
                        self.SHEET_CTYPES_TO_ATTRIBUTES_UNIQUE,
                        idx,
                        "WARNING: MP_ATTRIBUTE_ID is empty in current row",
                    )
                    continue
                ...
                data = {
                    ...
                    "field": attr_id,
                    "required": self.convert_value(
                        record.get("MP_ATTRIBUTE_IS_REQUIRED"), "bool"
                    ),
                    "read_only": self.convert_value(
                        record.get("MP_ATTRIBUTE_IS_READ_ONLY"), "bool"
                    ),
                    "activity": self.convert_value(
                        record.get("MP_ATTRIBUTE_IS_ACTIVITY"), "bool"
                    ),
                }
                ...
                self.log_to_sheet(
                    self.SHEET_CTYPES_TO_ATTRIBUTES_UNIQUE, idx, f"ERROR: {str(e)}"
                )
            ...
            try:
                ...
                if not list_id:
                    self.log_to_sheet(
                        self.SHEET_CTYPES,
                        idx,
                        "WARNING: MP_LIST_ID is empty in current row",
                    )
                    continue
                ...
                if response and response.get("id"):
                    self.update_sheet_cell(
                        self.SHEET_CTYPES, idx, "SYNC_DONE", "SUCCESS"
                    )
        ...
        for idx, record in enumerate(records, start=2):
            try:
                ...
                if not formula or formula.strip() == "":
                    self.log_to_sheet(
                        self.SHEET_CALCULATIONS, idx, "Skipped because formula is empty"
                    )
                    continue
